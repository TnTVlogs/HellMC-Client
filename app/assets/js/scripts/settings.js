...
/**
 * Prepare the Language Selector UI.
 */
function prepareLanguageSelector() {
    const selectedLang = ConfigManager.getLanguage() || 'es_ES'
    const selectedDiv = document.getElementById('settingsLanguageSelected')
    const optionsDiv = document.getElementById('settingsLanguageOptions')

    // Set initial text
    const initialOption = optionsDiv.querySelector(`.settingsSelectOption[value="${selectedLang}"]`)
    if (initialOption) {
        selectedDiv.innerHTML = initialOption.innerHTML
    }

    // Bind options
    for (let opt of optionsDiv.getElementsByClassName('settingsSelectOption')) {
        opt.onclick = (e) => {
            const val = e.target.getAttribute('value')
            if (val !== ConfigManager.getLanguage()) {
                langChanged = true
            }
            selectedDiv.innerHTML = e.target.innerHTML
            ConfigManager.setLanguage(val)
            ConfigManager.save()
        }
    }
}
...
/**
 * Enable or disable the language selector based on game status.
 * 
 * @param {boolean} running True if the game is running.
 */
function toggleLanguageLock(running) {
    const container = document.getElementById('settingsLanguageContainer')
    const statusLabel = document.getElementById('settingsLanguageLockStatus')
    const reasonLabel = document.getElementById('settingsLanguageLockReason')
    if (container) {
        if (running) {
            container.style.opacity = '0.5'
            container.style.pointerEvents = 'none'
            container.setAttribute('title', Lang.queryEJS('settings.languageLockedTooltip'))
